...
`* * *` | user | edit an existing person | update contact information upon changes
`* * *` | user | delete a person | remove entries that I no longer need
`* * *` | user | find a person by name | locate details of persons without having to go through the entire list
`* * *` | user | undo and redo commands | correct mistakes such as typo
`* *` | user | hide [private contact details](#private-contact-detail) by default | minimize chance of someone else seeing them by accident
`*` | user with many persons in the address book | sort persons by name | locate a person easily
`*` | user with many meetings with many people | add meeting details | check and arrange schedules easily
