...
`* * *` | user | update contact information of people |
`* * *` | user | find a person by name | locate details of persons without having to go through the entire list
`* *` | user | hide [private contact details](#private-contact-detail) by default | minimize chance of someone else seeing them by accident
`* *` | user | find a person by number | know who called me
`*` | user | print addressbook contacts to a printer | refer to contacts without the use of computers
`*` | user with many persons in the address book | sort persons by name | locate a person easily
`*` | secretive user | lock addressbook using a password | hide my contacts
`*` | user with many groups of friends | group contacts | easily associate a person and a group
