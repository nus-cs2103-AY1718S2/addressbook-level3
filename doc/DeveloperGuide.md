...
`* * *` | user | update a person | edit changes in the entries easily
`* * *` | user | find a person by name | locate details of persons without having to go through the entire list
`* * *` | user | clear the entire address book | remove all entries without having to manually delete every person in the list
`* *` | user | hide [private contact details](#private-contact-detail) by default | minimize chance of someone else seeing them by accident
`* *` | user | find people using tags | search for people that belongs to the same group easily
`*` | user with many persons in the address book | sort persons by name | locate a person easily
`*` | user | attach photo to a person  | personalize the entries
`*` | user | change the theme | customize it to my own preference
